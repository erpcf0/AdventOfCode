...
let result1 (data: string []) =
    data
    |> Array.sumBy (Set.count << Set.ofArray <<
        (Array.filter (fun y -> y <> '\n')) <<
        (fun x -> x.ToCharArray()))

let result2 (data: string []) =
    data
    |> Array.map ((Array.map (fun (y: string) ->
            y.ToCharArray() |> Set.ofArray)) << (fun x -> x.Split "\n"))
    |> Array.sumBy (Set.count << Array.reduce Set.intersect)

[<EntryPoint>]
let main argv =
    let data = input.Split "\n\n"
    printfn "%A" (data |> result2)
