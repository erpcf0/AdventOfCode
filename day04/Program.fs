...
hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021".Split "\n\n"
...
let transform (data: string) =
    let map = Map.empty
    data.Split [|' '; '\n'|]
    |> Array.fold (fun (m: Map<string, string>) x ->
        let temp = x.Split ':'
        m.Add (temp.[0], temp.[1])) map

let verify (map: Map<string, string>) =
    map.ContainsKey ("byr") &&
    map.ContainsKey ("iyr") &&
    map.ContainsKey ("eyr") &&
    map.ContainsKey ("hgt") &&
...
let deepverify (map: Map<string, string>) =
    Regex.IsMatch(map.Item "byr", "^(19[2-8][0-9]|199[0-9]|200[0-2])$") &&
    Regex.IsMatch(map.Item "iyr", "^(201[0-9]|2020)$") &&
    Regex.IsMatch(map.Item "eyr", "^(202[0-9]|2030)$") &&
    Regex.IsMatch(map.Item "hgt", "^(1[5-8][0-9]|19[0-3])cm|(59|6[0-9]|7[0-6])in$") &&
    Regex.IsMatch(map.Item "hcl", "^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$") &&
    Regex.IsMatch(map.Item "ecl", "^amb|blu|brn|gry|grn|hzl|oth$") &&
    Regex.IsMatch(map.Item "pid", "^\d{9}$")

let result1 =
    input
    |> Array.map (verify << transform)
    |> Array.filter ((=) true)
    |> Array.length
